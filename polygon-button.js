...
function App() {
    ...
    useEffect(() => {
      ...
      if (pollInfo.poll) {
        ...
        // TODO: Based on actual path
        fetch("http://localhost:3010/market/flowPurchase", { method: "POST", body: { tokenId: tokenId } })
          .then(res => {
            if (res.status === 202) {
              poll({ tokenId: tokenId })
                .then(async _ => {
                  setpollInfo(prev => ({ ...prev, poll: false, message: "Nft purchased on polygon" }))
                })
                .catch(async _ => {
                  setpollInfo(prev => ({ ...prev, poll: false, message: "", error: "Something went wrong while purchasing on polygon" }))
                })
            } else {
