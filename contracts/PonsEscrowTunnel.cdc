...
*/
pub contract PonsEscrowTunnelContract {
...
*/
	pub resource Escrow {
		...
		access(account) fun consummate (_ consummation : ((@EscrowResource): @EscrowResource)) : EscrowResourceDescription {
			...
			if ! PonsEscrowTunnelContract .satisfiesResourceDescription (
			...
			let fulfilledResourceDescription = PonsEscrowTunnelContract .resourceDescription (& consummationResource as &EscrowResource)

			// Transfer the obtained resources to the Escrow's fulfillment
			PonsEscrowTunnelContract .fullfillResource (<- consummationResource, self .fulfillment)
		...
		// 	PonsEscrowTunnelContract .fullfillResource (<- resources !, self .fulfillment) }

		init (id : String, resources : @EscrowResource, requirement : EscrowResourceDescription, fulfillment : EscrowFulfillment) {
			self .id = id

			self .heldResourceDescription = PonsEscrowTunnelContract .resourceDescription (& resources as &EscrowResource)
	...
	pub resource EscrowManager {
		...
		pub fun escrow (id : String) : &Escrow? {
			let escrowCapOptional = PonsEscrowTunnelContract .escrowCaps [id] 
	...
	/* Given an id and Escrow Capability, create a new Escrow and submit it to the PonsEscrowTunnelContract for consummation */
	...
	, resources : @EscrowResource, requirement : EscrowResourceDescription, fulfillment : EscrowFulfillment
		...
		let overwrittenEscrowCap = PonsEscrowTunnelContract .escrowCaps .insert (key: id, escrowCap)
	...
	access(account) fun consummateEscrow (id : String, consummation : ((@EscrowResource): @EscrowResource)) : Void {
		...
		let escrowRef = PonsEscrowTunnelContract .escrowCaps [id] !.borrow () !
 ...
 */
	...
	access(account) fun dismissEscrow (id : String) : Void {
		if ! PonsEscrowTunnelContract .escrowCaps .containsKey (id) {
			panic ("No active Escrow with the ID `" .concat (id) .concat ("` exists")) }
		let escrowCap = PonsEscrowTunnelContract .escrowCaps .remove (key: id) !
