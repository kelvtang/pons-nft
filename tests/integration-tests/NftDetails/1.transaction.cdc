...
, metadata : {String: String}
	...
	prepare (ponsAccount : AuthAccount, artistAccount : AuthAccount) {
        ...
        // 5) Test if we can get NFT serial ID and withdraw from collection using ID;
		// 6) Destroy the NFT
        ...
        let serialId = PonsTunnelContract .getNftSerialId (nftId: mintId, collector: ponsAccount);

        let des_nft <- PonsUsage .borrowOwnPonsCollection(collector: ponsAccount) .withdraw(withdrawID: serialId);
