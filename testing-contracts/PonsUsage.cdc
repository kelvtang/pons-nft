...
pub contract PonsUsage {
	...
	pub fun purchaseUsingVaultFusd (patron : AuthAccount, nftId : String, _ purchaseVault : @FungibleToken.Vault) : Void {
		...
		// Deposit the purchased NFT in the patron's Pons collection
		PonsUsage .borrowOwnPonsCollection (collector: patron) .depositNft (<- nft) }
	...
	pub fun escrowCapabilityPathFlow (_ account : AuthAccount, _ id : String) : CapabilityPath {
	...
	/* Get a free Capability Path to store a Capability to an Escrow */
	pub fun escrowCapabilityPathFusd (_ account : AuthAccount, _ id : String) : CapabilityPath {
		// This function is not yet defined in this version of Cadence
		//return PrivatePath ("escrow__" .concat (id)) !
		let potentialCapabilityPaths =
			[ /private/escrow_fusd__1, /private/escrow_fusd__2, /private/escrow_fusd__3, /private/escrow_fusd__4, /private/escrow_fusd__5, /private/escrow_fusd__6, /private/escrow_fusd__7, /private/escrow_fusd__8, /private/escrow_fusd__9, /private/escrow_fusd__10 ]
		for capabilityPath in potentialCapabilityPaths {
			if account .getCapability <&PonsEscrowContractFusd.Escrow> (capabilityPath) .check () {
				let storagePath = account .getLinkTarget (capabilityPath) ! as! StoragePath
				let escrowRefOptional = account .borrow <&PonsEscrowContractFusd.Escrow> (from: storagePath) 
				if escrowRefOptional == nil {
					account .unlink (capabilityPath)
					return capabilityPath }
				else {
					if escrowRefOptional !.isReleased () {
						account .unlink (capabilityPath)
						destroy account .load <@PonsEscrowContractFusd.Escrow> (from: storagePath)
						return capabilityPath } } }
			else {
				return capabilityPath } }
		panic ("No free escrow capability paths found") }
	...
	pub fun escrowStoragePathFlow (_ account : AuthAccount, _ id : String) : StoragePath {
	...
	/* Get a free Storage Path to store a Capability to an Escrow */
	pub fun escrowStoragePathFusd (_ account : AuthAccount, _ id : String) : StoragePath {
		//return StoragePath ("escrow__" .concat (id)) !
		let potentialStoragePaths =
			[ /storage/escrow_fusd__1, /storage/escrow_fusd__2, /storage/escrow_fusd__3, /storage/escrow_fusd__4, /storage/escrow_fusd__5, /storage/escrow_fusd__6, /storage/escrow_fusd__7, /storage/escrow_fusd__8, /storage/escrow_fusd__9, /storage/escrow_fusd__10 ]
		for storagePath in potentialStoragePaths {
			let escrowRefOptional = account .borrow <&PonsEscrowContractFusd.Escrow> (from: storagePath) 
			if escrowRefOptional == nil {
				return storagePath }
	...
	, requirement : PonsEscrowContract.EscrowResourceDescription, fulfillment : PonsEscrowContract.EscrowFulfillment
		...
		let capabilityPath = PonsUsage .escrowCapabilityPathFlow (submitter, id)
		let storagePath = PonsUsage .escrowStoragePathFlow (submitter, id)
	...
	/* Submit an escrowing using the specified id, resources, requirement, and fulfillment */
	pub fun submitEscrowUsingFusd
	( submitter : AuthAccount, id : String
	, resources : @PonsEscrowContractFusd.EscrowResource
	, requirement : PonsEscrowContractFusd.EscrowResourceDescription, fulfillment : PonsEscrowContractFusd.EscrowFulfillment
	) : StoragePath {
		// Obtain escrow capability and storage paths
		let capabilityPath = PonsUsage .escrowCapabilityPathFusd (submitter, id)
		let storagePath = PonsUsage .escrowStoragePathFusd (submitter, id)

		// Create an escrow capability to the escrow storage path
		let escrowCap = submitter .link <&PonsEscrowContractFusd.Escrow> (capabilityPath, target: storagePath) !

		// First, submit an escrow with the specified information and resources, obtaining an Escrow resource
		// Then, save the Escrow resource to the arranged storage path
		submitter .save (
			<- PonsEscrowContractFusd .submitEscrow (
				id: id, escrowCap: escrowCap, resources: <- resources,
				requirement: requirement, fulfillment: fulfillment ),
			to: storagePath )
	...
	, heldResourceDescription : PonsEscrowContract.EscrowResourceDescription, requirement : PonsEscrowContract.EscrowResourceDescription
		...
		return PonsUsage .submitEscrowUsingFlow (
			submitter: submitter, id: id,
			resources: <- heldResources, requirement: requirement,
			fulfillment: fulfillment ) }
	/* Submit an escrow using the specified id and requirement, gathering the escrow resources and fulfillment from the default paths */
	pub fun submitEscrowFusd
	( submitter : AuthAccount, id : String
	, heldResourceDescription : PonsEscrowContractFusd.EscrowResourceDescription, requirement : PonsEscrowContractFusd.EscrowResourceDescription
	) : StoragePath {
		// Ensure the submitter has a Flow Vault and a PonsCollection, constructing an EscrowFulfillment using the two
		let fulfillment =
			PonsEscrowContractFusd.EscrowFulfillment (
				receivePaymentCap: PonsUsage .prepareFusdCapability (account: submitter),
				receiveNftCap: PonsUsage .preparePonsNftReceiverCapability (collector: submitter) )
		...
		// Create EscrowResource based on the withdrawn Flow Vault and Pons NFTs
		var heldResources <-
			PonsEscrowContractFusd .makeEscrowResource (fusdVault: <- heldFusdVault, ponsNfts: <- heldPonsNfts)

		// Submit the obtained EscrowFulfillment and EscrowResource for escrow
		return PonsUsage .submitEscrowUsingFusd (
			submitter: submitter, id: id,
			resources: <- heldResources, requirement: requirement,
			fulfillment: fulfillment ) }
	}
